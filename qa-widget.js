  ...
  if (document.URL.indexOf("optimizely_qa=true") > -1) {
    ...
    if ($("#qabanner").length < 1) {
      ...
      if (localStorage.getItem("optimizely_qa_left") !== null) {
        $("#qabanner").css({"left":localStorage.getItem("optimizely_qa_left"), "top": localStorage.getItem("optimizely_qa_top")});
      }
    ...
    $(document).ready(function () {
      for (var idx = 0; idx < window.QA_EXPERIMENTS.length; idx++) {
        ...
        for (i = 0; i < optimizely.data.experiments[eid].variation_ids.length; i++) {
          ...
          var selected = optimizely.variationMap[eid] == i ? "selected" : "";
          v += "<a class='vswitch " + selected + "' title='" + optimizely.data.variations[optimizely.data.experiments[eid].variation_ids[i]].name + "'>" + i + "</a>, ";
        }
        ...
        for (i = 0; i < optimizely.activeExperiments.length; i++) { 
          if (optimizely.activeExperiments[i] == eid) { active = true; }
        }
        if (!active) { 
